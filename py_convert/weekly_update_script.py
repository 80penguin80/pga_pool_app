...
for i in df['official_money']:
    try:
        whole_number.append(int(float(i)))  
    except:
        pass
...
def get_tournament_results():
    ...
    cur = conn.cursor()
    cur.execute("TRUNCATE TABLE public.tournament_results")

    # Define the insert query
    insert_query = """
        INSERT INTO public.tournament_results (date, tournament_name, tournament_id, player_name, position, score, 
                                                r1, r2, r3, r4, fedex_cup_points, official_money)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
    """

    # Check if the DataFrame columns match the expected number of placeholders
    expected_columns = 12
    if df.shape[1] != expected_columns:
        raise ValueError(f"DataFrame has {df.shape[1]} columns, but {expected_columns} columns are expected.")
...
def update_weekly_results():
    ...
    try:
        ...
        sql_query = '''
        INSERT INTO public.weekly_tournament_results (tournament_name, user_name, player_pick, prize_money, date)
        SELECT DISTINCT
